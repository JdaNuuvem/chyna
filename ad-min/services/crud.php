	...
	if ($gamebco && isset($gamebco['distribution'])) {
		switch ($gamebco['distribution']) {
			case 'fiverscan':
				...
				$stmtUsr = $pdo->prepare("SELECT * FROM usuarios WHERE id = :id");
				$stmtUsr->execute(['id' => $mobile]); // $mobile é na verdade id
				...
				if ($usuario) {
					...
					if ($fiverscan) {
						...
						// Log para debug
						file_put_contents(__DIR__ . '/../../pegarLinkJogo.log', "Fiverscan request: " . $json_data . "\nResponse: " . $response . "\nDecoded: " . json_encode($data) . "\nGames: " . json_encode($games) . "\n", FILE_APPEND);
					} else {
			...
			case 'playfiver':
				$stmtUsr = $pdo->prepare("SELECT * FROM usuarios WHERE id = :id");
				$stmtUsr->execute(['id' => $mobile]); // $mobile é id
				$usuario = $stmtUsr->fetch();
