...
function getAuthToken()
{
    ...
    // Se não encontrou no cabeçalho, tenta do cookie
    if (isset($_COOKIE['token_user']) && !empty($_COOKIE['token_user'])) {
        return $_COOKIE['token_user'];
    }
    return null;
}
    ...
    case 'GET':
        ...
        if (parse_url($requestURI, PHP_URL_PATH) === '/api/china/launch/') {
            ...
            if (!empty($code) && !empty($game_type)) {
                ...
                $qry = "SELECT * FROM usuarios WHERE token='" . mysqli_real_escape_string($mysqli, $token) . "'";
                $resp = mysqli_query($mysqli, $qry);
                ...
                if (mysqli_num_rows($resp) > 0) {
                    ...
                    if ($gameretur && isset($gameretur['gameURL'])) {
                        // Monta a resposta
                        $response = array(
                            "status" => true,
                        ...
                        $response = array(
                            "status" => 0,
                            "msg" => "Erro ao obter link do jogo",
                        );
                    }
                } else {
                    $response = array(
